...
const SignInForm = ({ firebase, onClick }) => {
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    firebase
      .doSignInWithEmailAndPassword(credential.email, credential.password)
      .catch((error) => {
        setError(error);
        setInputClass(
  ...
  return (
    <div className="flex flex-col items-center space-y-4">
      <h3 className="text-2xl font-jetbrains-mono">Welcome Back Master</h3>
      <p>All your saved gradient in one place</p>
      <form
